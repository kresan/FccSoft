...
// ------   Constructor   --------------------------------------------------
FccFieldPar::FccFieldPar(const char* name, const char* title,
			 const char* context) 
...
// ------   Put parameters   -----------------------------------------------
void FccFieldPar::putParams(FairParamList* list) {
...
// --------   Get parameters   ---------------------------------------------
Bool_t FccFieldPar::getParams(FairParamList* list) {
...
void FccFieldPar::SetParameters(FairField* field) {

  if ( ! field ) {
    cerr << "-W- FccFieldPar::SetParameters: Empty field pointer!" << endl;
    return;
  }
  ...
  if ( fType == 0 ) {                                 // constant field
    FccConstField* fieldConst = (FccConstField*) field;
    ...
    cerr << "-W- FccFieldPar::SetParameters: Unknown field type "
	 << fType << "!" << endl;
    fBx = fBy = fBz = 0.;
