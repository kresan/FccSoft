...
#include "FccMCTrack.h"                // for FccMCTrack
...
// -----   Default constructor   -------------------------------------------
FccStack::FccStack(Int_t size)
  : FairGenericStack(),
    fStack(),
    fParticles(new TClonesArray("TParticle", size)),
    fTracks(new TClonesArray("FccMCTrack", size)),
    fStoreMap(),
    fStoreIter(),
    fIndexMap(),
    fIndexIter(),
...
void FccStack::PushTrack(Int_t toBeDone, Int_t parentId, Int_t pdgCode,
...
TParticle* FccStack::PopNextTrack(Int_t& iTrack)
{
...
TParticle* FccStack::PopPrimaryForTracking(Int_t iPrim)
{
  ...
  if (iPrim < 0 || iPrim >= fNPrimaries) {
    fLogger->Fatal(MESSAGE_ORIGIN, "FccStack: Primary index out of range! %i ", iPrim );
    Fatal("FccStack::PopPrimaryForTracking", "Index out of range");
  }
  ...
  if ( ! (part->GetMother(0) < 0) ) {
    fLogger->Fatal(MESSAGE_ORIGIN, "FccStack:: Not a primary track! %i ",iPrim);
    Fatal("FccStack::PopPrimaryForTracking", "Not a primary track");
  }
...
TParticle* FccStack::GetCurrentTrack() const
{
  TParticle* currentPart = GetParticle(fCurrentTrack);
  if ( ! currentPart) {
    fLogger->Warning(MESSAGE_ORIGIN,"FccStack: Current track not found in stack!");
    Warning("FccStack::GetCurrentTrack", "Track not found in stack");
  }
  return currentPart;
}
// -------------------------------------------------------------------------
...
void FccStack::AddParticle(TParticle* oldPart)
{
...
void FccStack::FillTrackArray()
{

  fLogger->Debug(MESSAGE_ORIGIN, "FccStack: Filling MCTrack array...");
  ...
  for (Int_t iPart=0; iPart<fNParticles; iPart++) {
    ...
    if (fStoreIter == fStoreMap.end() ) {
      fLogger->Fatal(MESSAGE_ORIGIN, "FccStack: Particle %i not found in storage map! ", iPart);
      Fatal("FccStack::FillTrackArray",
    ...
    if (store) {
      FccMCTrack* track =
        new( (*fTracks)[fNTracks]) FccMCTrack(GetParticle(iPart));
      fIndexMap[iPart] = fNTracks;
      // --> Set the number of points in the detectors for this track
      for (Int_t iDet=kFccVxd; iDet<kSTOPHERE; iDet++) {
...
void FccStack::UpdateTrackIndex(TRefArray* detList)
{

  fLogger->Debug(MESSAGE_ORIGIN, "FccStack: Updating track indizes...");
  Int_t nColl = 0;

  // First update mother ID in MCTracks
  for (Int_t i=0; i<fNTracks; i++) {
    FccMCTrack* track = (FccMCTrack*)fTracks->At(i);
    Int_t iMotherOld = track->GetMotherId();
    fIndexIter = fIndexMap.find(iMotherOld);
    if (fIndexIter == fIndexMap.end()) {
      fLogger->Fatal(MESSAGE_ORIGIN, "FccStack: Particle index %i not found in dex map! ", iMotherOld);
      Fatal("FccStack::UpdateTrackIndex", "Particle index not found in map");
    }
  ...
  while( (det = (FairDetector*)fDetIter->Next() ) ) {
    ...
    while ( (hitArray = det->GetCollection(iColl++)) ) {
      ...
      for (Int_t iPoint=0; iPoint<nPoints; iPoint++) {
        ...
        if (fIndexIter == fIndexMap.end()) {
          fLogger->Fatal(MESSAGE_ORIGIN, "FccStack: Particle index %i not found in index map! ", iTrack);
          Fatal("FccStack::UpdateTrackIndex", "Particle index not found in map");
        }
...
// -----   Public method Print  --------------------------------------------
void FccStack::Print(Int_t iVerbose) const
{
  cout << "-I- FccStack: Number of primaries        = "
       << fNPrimaries << endl;
  ...
  if (iVerbose) {
    for (Int_t iTrack=0; iTrack<fNTracks; iTrack++) {
      ((FccMCTrack*) fTracks->At(iTrack))->Print(iTrack);
    }
  }
}
// -------------------------------------------------------------------------
...
void FccStack::AddPoint(DetectorId detId)
{
  Int_t iDet = detId;
...
void FccStack::AddPoint(DetectorId detId, Int_t iTrack)
{
  if ( iTrack < 0 ) { return; }
  Int_t iDet = detId;
...
Int_t FccStack::GetCurrentParentTrackNumber() const
{
...
TParticle* FccStack::GetParticle(Int_t trackID) const
{
  if (trackID < 0 || trackID >= fNParticles) {
    fLogger->Debug(MESSAGE_ORIGIN, "FccStack: Particle index %i out of range.",trackID);
    Fatal("FccStack::GetParticle", "Index out of range");
  }
