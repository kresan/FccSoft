...
/** FccMCTrack.h
 ** Data class for storing Monte Carlo tracks processed by the FccStack.
 ** A FccMCTrack can be a primary track put into the simulation or a
 ** secondary one produced by the transport through decay or interaction.
 **/


#ifndef FccMCTrack_H
#define FccMCTrack_H 1

#include "TObject.h"                    // for TObject
#include "FccDetectorList.h"         // for DetectorId
...
class FccMCTrack : public TObject
{
  ...
  public:
    ...
    FccMCTrack(Int_t pdgCode, Int_t motherID, Double_t px, Double_t py,
                Double_t pz, Double_t x, Double_t y, Double_t z,
                Double_t t, Int_t nPoints);

    /**  Copy constructor  **/
    FccMCTrack(const FccMCTrack& track);


    /**  Constructor from TParticle  **/
    FccMCTrack(TParticle* particle);
...
inline Double_t FccMCTrack::GetEnergy() const
{
...
inline void FccMCTrack::GetMomentum(TVector3& momentum)
{
  momentum.SetXYZ(fPx,fPy,fPz);
}


inline void FccMCTrack::Get4Momentum(TLorentzVector& momentum)
{
  momentum.SetXYZT(fPx,fPy,fPz,GetEnergy());
}


inline void FccMCTrack::GetStartVertex(TVector3& vertex)
{
